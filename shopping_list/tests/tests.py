...
def test_client_retrieves_only_shopping_lists_they_are_member_of(create_user, create_authenticated_client):
    ...
    url = reverse("all-shopping-lists")
    ShoppingList.objects.create(name="Books").members.add(user)
    ShoppingList.objects.create(name="Groceries").members.add(user)
    ...
    assert response.data["results"][1]["name"] == "Books"
...
@pytest.mark.django_db
def test_admin_can_add_shopping_item(create_user, create_shopping_list, admin_client):
    user = create_user()
    shopping_list = create_shopping_list(user)
...
@pytest.mark.django_db
def test_not_member_can_not_retieve_shopping_items(create_user, create_authenticated_client, create_shopping_item):
...
@pytest.mark.django_db
def test_add_member_list_member(create_user, create_authenticated_client, create_shopping_list):
    user = create_user()
    client = create_authenticated_client(user)
    shopping_list = create_shopping_list(user)
